...
const AdminDashboard = ({ adminStats, setActiveTab, fleetTrucks, pickups, completedPickups }) => {
  ...
  <div className="dashboard-grid animate-fade-in" style={{ gridTemplateColumns: 'repeat(12, 1fr)', gap: '1.5rem' }}>
    ...
    {/* KPI Cards */}
    <div className="glass-panel stat-card" onClick={() => setActiveTab('fleet')} style={{ gridColumn: 'span 3', cursor: 'pointer', borderTop: '4px solid var(--success)', padding: '1.5rem', textAlign: 'center' }}>
      <Truck size={28} color="var(--success)" style={{ marginBottom: '0.5rem' }} />
      <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px', marginBottom: '0.3rem' }}>ACTIVE TRUCKS</p>
      <h3 style={{ fontSize: '2.5rem', color: 'var(--success)' }}>{activeTrucks}</h3>
    </div>
    <div className="glass-panel stat-card" onClick={() => setActiveTab('fleet')} style={{ gridColumn: 'span 3', cursor: 'pointer', borderTop: '4px solid var(--warning)', padding: '1.5rem', textAlign: 'center' }}>
      <Truck size={28} color="var(--warning)" style={{ marginBottom: '0.5rem' }} />
      <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px', marginBottom: '0.3rem' }}>IDLE / MAINTENANCE</p>
      <h3 style={{ fontSize: '2.5rem', color: 'var(--warning)' }}>{idleTrucks + maintenanceTrucks}</h3>
    </div>
    <div className="glass-panel stat-card" onClick={() => setActiveTab('analytics')} style={{ gridColumn: 'span 3', cursor: 'pointer', borderTop: '4px solid var(--primary)', padding: '1.5rem', textAlign: 'center' }}>
      <MapPin size={28} color="var(--primary)" style={{ marginBottom: '0.5rem' }} />
      <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px', marginBottom: '0.3rem' }}>PENDING PICKUPS</p>
      <h3 style={{ fontSize: '2.5rem', color: 'var(--primary)' }}>{pendingPickups}</h3>
    </div>
    <div className="glass-panel stat-card" onClick={() => setActiveTab('analytics')} style={{ gridColumn: 'span 3', cursor: 'pointer', borderTop: '4px solid rgb(139, 92, 246)', padding: '1.5rem', textAlign: 'center' }}>
      <CheckCircle size={28} color="rgb(139, 92, 246)" style={{ marginBottom: '0.5rem' }} />
      <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px', marginBottom: '0.3rem' }}>COMPLETED TODAY</p>
      <h3 style={{ fontSize: '2.5rem', color: 'rgb(139, 92, 246)' }}>{totalCompleted}</h3>
    </div>
    ...
    <div className="glass-panel" style={{ gridColumn: 'span 4', padding: '2rem' }}>
      ...
      <div style={{ marginTop: '1.5rem', padding: '1rem', background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ flex: 1, height: '10px', background: 'var(--bg-surface)', borderRadius: '5px', overflow: 'hidden' }}>
            <div style={{ width: `${avgCapacity}%`, height: '100%', background: avgCapacity > 80 ? 'var(--danger)' : avgCapacity > 50 ? 'var(--warning)' : 'var(--success)', borderRadius: '5px', transition: 'width 0.5s ease' }}></div>
          </div>
          <span style={{ fontWeight: 'bold', color: 'var(--text-main)' }}>{avgCapacity}%</span>
        </div>
      </div>
    </div>
          ...
          {completedPickups.slice(0, 6).map((p, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'var(--bg-card)', borderRadius: '10px', borderLeft: '3px solid var(--success)' }}>
              <div>
                ...
                <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{p.type} • {p.address?.split(',')[0]}</p>
              </div>
...
function App() {
  ...
  const renderMainContent = () => {
    switch (userProfile.role) {
      ...
      case 'Admin':
        if (activeTab === 'dashboard') return <AdminDashboard adminStats={adminStats} setActiveTab={changeTab} fleetTrucks={fleetTrucks} pickups={pickups} completedPickups={completedPickups} />;
        ...
        return <AdminDashboard adminStats={adminStats} setActiveTab={changeTab} fleetTrucks={fleetTrucks} pickups={pickups} completedPickups={completedPickups} />;
